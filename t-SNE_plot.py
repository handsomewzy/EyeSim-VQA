...
def load_train_test_split(train_dataset_path, test_dataset_path, train_ann_file, test_ann_file):
    """
    读取训练集和测试集标注文件，并返回 train_video_infos, test_video_infos 两个列表。

    Args:
        dataset_path (str): 数据集根路径
        train_ann_file (str): 训练集标注文件路径
        test_ann_file (str): 测试集标注文件路径

    Returns:
        tuple: (train_video_infos, test_video_infos)
    """
    def read_annotation_file(file_path,dataset_path):
        video_infos = []
        with open(file_path, "r") as fin:
            for line in fin:
                ...
                filename, _, _, label = line_split
                label = float(label)
                full_path = osp.join(dataset_path, filename)
                video_infos.append(dict(filename=full_path, label=label))
        return video_infos

    train_video_infos = read_annotation_file(train_ann_file,train_dataset_path)
    test_video_infos = read_annotation_file(test_ann_file,test_dataset_path)
    return train_video_infos, test_video_infos
...
def finetune_epoch_tSNE(
    ...
    model.train()
    # 初始化缓存
    all_feat_aes, all_clean_aes, all_feat_tec, all_vsr_tec = [], [], [], []
    mos_feat_aes, mos_clean_aes, mos_feat_tec, mos_vsr_tec = [], [], [], []

    for data in tqdm(ft_loader, desc="Collecting features"):
        video = {k: data[k].to(device) for k in sample_types if k in data}
        y = data["gt_label"].float().detach().cpu().numpy().reshape(-1)

        # 获取四类特征
        feat_aes, clean_feat_aes, feat_tec, vsr_feat_tec = model.tSNE_feature(video, inference=True, reduce_scores=False)

        # 展平
        feat_aes = feat_aes.reshape(feat_aes.size(0), -1).detach().cpu().numpy()
        clean_feat_aes = clean_feat_aes.reshape(clean_feat_aes.size(0), -1).detach().cpu().numpy()
        feat_tec = feat_tec.reshape(feat_tec.size(0), -1).detach().cpu().numpy()
        vsr_feat_tec = vsr_feat_tec.reshape(vsr_feat_tec.size(0), -1).detach().cpu().numpy()

        # 累加
        all_feat_aes.append(feat_aes)
        all_clean_aes.append(clean_feat_aes)
        all_feat_tec.append(feat_tec)
        all_vsr_tec.append(vsr_feat_tec)

        mos_feat_aes.extend(y)
        mos_clean_aes.extend(y)
        mos_feat_tec.extend(y)
        mos_vsr_tec.extend(y)

    # 合并
    feat_aes_np = np.concatenate(all_feat_aes, axis=0)
    clean_aes_np = np.concatenate(all_clean_aes, axis=0)
    feat_tec_np = np.concatenate(all_feat_tec, axis=0)
    vsr_tec_np = np.concatenate(all_vsr_tec, axis=0)

    mos_feat_aes = np.array(mos_feat_aes)
    mos_clean_aes = np.array(mos_clean_aes)
    mos_feat_tec = np.array(mos_feat_tec)
    mos_vsr_tec = np.array(mos_vsr_tec)

    print(feat_aes_np.shape,mos_feat_aes.shape)

    # PCA 降维
    print("[PCA] Reducing all four branches to 128 dims...")
    n_components = min(128, feat_aes_np.shape[0])  # 这里 shape[0] 是 12
    feat_aes_pca = PCA(n_components=n_components).fit_transform(feat_aes_np)
    clean_aes_pca = PCA(n_components=n_components).fit_transform(clean_aes_np)
    feat_tec_pca = PCA(n_components=n_components).fit_transform(feat_tec_np)
    vsr_tec_pca = PCA(n_components=n_components).fit_transform(vsr_tec_np)

    # 保存
    save_dir = "tsne_features_4branches"
    os.makedirs(save_dir, exist_ok=True)

    np.save(os.path.join(save_dir, "feat_aes_pca.npy"), feat_aes_pca)
    np.save(os.path.join(save_dir, "clean_aes_pca.npy"), clean_aes_pca)
    np.save(os.path.join(save_dir, "feat_tec_pca.npy"), feat_tec_pca)
    np.save(os.path.join(save_dir, "vsr_tec_pca.npy"), vsr_tec_pca)

    np.save(os.path.join(save_dir, "mos_feat_aes.npy"), mos_feat_aes)
    np.save(os.path.join(save_dir, "mos_clean_aes.npy"), mos_clean_aes)
    np.save(os.path.join(save_dir, "mos_feat_tec.npy"), mos_feat_tec)
    np.save(os.path.join(save_dir, "mos_vsr_tec.npy"), mos_vsr_tec)

    print("✅ All four feature branches saved to:", save_dir)
...
def main():
    ...
    parser.add_argument(
        "-o", "--opt", type=str, default="proposed.yml", help="the option file"
    )
    ...
    for split in range(10):
        ...
        if opt.get("split_seed", -1) > 0:
            ...
            # 读取两个分开的数据集，分别是训练和测试
            # split_duo = load_train_test_split(
            #     train_dataset_path='/data1/userhome/luwen/Code/wzy/VQA_dataset/KVQ/train_video-001',
            #     test_dataset_path='/data1/userhome/luwen/Code/wzy/VQA_dataset/KVQ/Validation/val_videos',
            #     train_ann_file='/data1/userhome/luwen/Code/wzy/DOVER-master/examplar_data_labels/KVQ/KVQ_train.txt',
            #     test_ann_file='/data1/userhome/luwen/Code/wzy/DOVER-master/examplar_data_labels/KVQ/KVQ_val.txt'
            # ) # KVQ 数据集
            # (
            #     opt["data"]["train"]["args"]["anno_file"],
            #     opt["data"]["eval"]["args"]["anno_file"],
        ...
        for epoch in range(opt["num_epochs"]):
            ...
            for key in val_loaders:
                bests[key] = inference_set(
                    ...
                    bests[key],
                    save_model=opt["save_model"],
                    save_name=opt["name"] + "_all_" + args.target_set + f"_{split}",
                    suffix=key + "_s",
                    save_type="full",
                )
                if model_ema is not None:
