...
def finetune_epoch(
    ...
    for i, data in enumerate(tqdm(ft_loader, desc=f"Training in epoch {epoch}")):
        ...
        if len(scores) > 1:
            ...
            y_pred = scores[0]
        # 这个是旧的，输出的五维度的数据，求后面三个的均值，有的头直接就弄出结果了，不需要再均值了
        ...
        if need_separate_sup:
            ...
            p_loss_a = plcc_loss(scores[0], y)
            p_loss_b = plcc_loss(scores[1], y)
            r_loss_a = rank_loss(scores[0], y)
            r_loss_b = rank_loss(scores[1], y)
