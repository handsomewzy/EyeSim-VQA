...
def finetune_epoch(
    ...
    for i, data in enumerate(tqdm(ft_loader, desc=f"Training in epoch {epoch}")):
        ...
        for i, s in enumerate(scores):
            print(f"[score {i}] shape: {s.shape}")
            # [score 0] shape: torch.Size([4, 1, 16, 7, 7])
            # [score 1] shape: torch.Size([4, 1, 48, 7, 7])
        if len(scores) > 1:
...
def main():
    ...
    parser.add_argument(
        "-t", "--train", type=str, default="train-ltrain", help="training set"
    )
    
    parser.add_argument(
        "-v", "--val", type=str, default="val-ltest", help="training set"
    )
