...
class ViewDecompositionDataset_VE(torch.utils.data.Dataset):
    def __init__(self, opt):
        ## opt is a dictionary that includes options for video sampling
        ...
        self.video_infos = []
        self.expanded_video_infos = []
        self.distorted_video_infos = []
        self.ann_file = opt["anno_file"]
        self.data_prefix = opt["data_prefix"]
        self.opt = opt
        self.sample_types = opt["sample_types"]
        ...
        if isinstance(self.ann_file, list):
            ...
            try:
                with open(self.ann_file, "r") as fin:
                    lines = fin.readlines()
                    print(f"📄 加载标注文件 {self.ann_file}，共 {len(lines)} 行")

                    for line in lines:
                        line_split = line.strip().split(",")
                        if len(line_split) != 4:
                            print(f"⚠️ 标注格式错误，跳过: {line.strip()}")
                            continue
                ...
                for (root, dirs, files) in os.walk(self.data_prefix, topdown=True):
                    for file in files:
                        if file.endswith(".mp4"):
                            video_filenames.append(os.path.join(root, file))
                print(f"🔍 无标注，共找到 {len(video_filenames)} 个视频")
                video_filenames = sorted(video_filenames)
                for filename in video_filenames:
                    self.video_infos.append(dict(filename=filename, label=-1))

        # ✅ 构造扩展样本
        distortion_types = ["gaussian_noise", "motion_blur", "jpeg_compression", "salt_pepper", "mean_blur"]

        for _ in range(3):  # 扩展三倍
            for info in self.video_infos:
                distortion = random.choice(distortion_types)

                # 原始
                self.expanded_video_infos.append(dict(
                    filename=info["filename"],
                    label=info["label"]
                ))

                # 失真路径
                distorted_path = info["filename"].replace(
                    "KoNViD_1k_videos", f"KoNViD_1k_video_distorted/{distortion}"
                )
                self.distorted_video_infos.append(dict(
                    filename=distorted_path,
                    label=info["label"]
                ))

        print(f"✅ 成功构造 {len(self.expanded_video_infos)} 对原始+失真样本")

    def __getitem__(self, index):
        video_info = self.expanded_video_infos[index]                # 原始视频
        distorted_info = self.distorted_video_infos[index]           # 对应的失真视频

        filename_orig = video_info["filename"]
        filename_dist = distorted_info["filename"]
        label = video_info["label"]

        try:
            # 读取原始视频帧
            data_orig, frame_inds = spatial_temporal_view_decomposition(
                filename_orig,
                self.sample_types,
                self.samplers,
                self.phase == "train",
            ...
            # 读取失真视频帧（保持与原视频同样的帧采样策略）
            data_dist, _ = spatial_temporal_view_decomposition(
                filename_dist,
                self.sample_types,
                self.samplers,
                self.phase == "train",
            ...
            for k, v in data_orig.items():
                data_orig[k] = ((v.permute(1, 2, 3, 0) - self.mean) / self.std).permute(3, 0, 1, 2)

            # Normalize 失真视频
            for k, v in data_dist.items():
                data_dist[k] = ((v.permute(1, 2, 3, 0) - self.mean) / self.std).permute(3, 0, 1, 2)

            # 构造输出字典
            data = {
                "orig_video": data_orig,
                "distorted_video": data_dist,
                "gt_label": label,
                "frame_inds": frame_inds,
                "name_orig": filename_orig,
                "name_dist": filename_dist,
            }
            ...
            print(f"❌ 读取失败: {filename_orig} or {filename_dist}: {str(e)}")
            return {"name_orig": filename_orig, "name_dist": filename_dist}

        return data

    def __len__(self):
        return len(self.expanded_video_infos)