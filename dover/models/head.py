...
class VQAHead_DualWeightPlus(nn.Module):
    """Advanced MLP Regression Head with dual-branch weighted fusion and sparsity regularization."""
    ...
    def __init__(
        ...
        dropout_ratio=0.5,
        pre_pool=False,
        weight_sparsity_lambda=0.001,  # 稀疏正则项权重
    ):
        super().__init__()
        self.in_channels = in_channels
        self.hidden_channels = hidden_channels
        ...
        self.pre_pool = pre_pool
        self.dropout_ratio = dropout_ratio
        self.weight_sparsity_lambda = weight_sparsity_lambda
        ...
        self.fc_score = nn.Conv3d(self.hidden_channels, num_class, (1, 1, 1))
        self.fc_weight = nn.Conv3d(self.hidden_channels, num_class, (1, 1, 1))
        self.sigmoid = nn.Sigmoid()

    def forward(self, x, rois=None, return_aux_loss=False):
        # x: [B, C, T, H, W]
        if self.pre_pool:
            x = self.avg_pool(x)

        x = self.gelu(self.fc_hid(x))

        score_map = self.fc_score(self.dropout(x))       # [B, 1, T, H, W]
        weight_map = self.sigmoid(self.fc_weight(self.dropout(x)))  # [B, 1, T, H, W]

        weighted_score_map = score_map * weight_map       # [B, 1, T, H, W]

        if return_aux_loss:
            # 返回额外的 weight sparsity loss，用于训练加正则
            sparsity_loss = weight_map.mean()
            return weighted_score_map, self.weight_sparsity_lambda * sparsity_loss
        else:
            return weighted_score_map
    

class VQAHead_DualWeight(nn.Module):
    """Enhanced MLP Regression Head for VQA with dual-branch weighted fusion.
    """

    def __init__(
        self, in_channels=768, hidden_channels=64, num_class=1,
    ...
    def forward(self, x, rois=None):
        ...
        x = self.gelu(self.fc_hid(x))  # [B, hidden_channels, T, H, W]

        score_map = self.fc_score(self.dropout(x))       # [B, 1, T, H, W]
        weight_map = self.sigmoid(self.fc_weight(self.dropout(x)))  # [B, 1, T, H, W]

        # 直接输出加权后的局部得分，而不是整体归一化
        weighted_score_map = score_map * weight_map  # [B, 1, T, H, W]
