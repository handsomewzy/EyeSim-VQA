...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                (SwinTransformerBlock3D_Mamba if use_mamba else SwinTransformerBlock3D)(
                    dim=dim,
                    num_heads=num_heads,
                    window_size=window_size,
                    shift_size=(0, 0, 0) if (i % 2 == 0) else self.shift_size,
                    ...
                    jump_attention=jump_attention,
                    frag_bias=frag_bias,
                    use_mamba_first=True,  # ✨ 如果是Mamba版，还能传入先Mamba后Attention的顺序
                )
                for i in range(depth)
            ]
        )
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(
        ...
        for i_layer in range(self.num_layers):
            # 判断当前层是否使用mamba
            mamba_layers = 1
            use_mamba_layer = use_mamba and (i_layer < mamba_layers)  
            # mamba_layers 是一个新的超参数，比如 1 或 2
            ...
            layer = BasicLayer(
                ...
                jump_attention=jump_attention[i_layer],
                frag_bias=frag_biases[i_layer],
                use_mamba=use_mamba_layer,  # 注意这里！按层开关
            )
            self.layers.append(layer)
...
def swin_3d_mamba(**kwargs):
    # Original Swin-3D Small with reduced windows
    return SwinTransformer3D(use_mamba=True, **kwargs)
...
class MinimalMambaBlock3D(nn.Module):
    """Fast vectorized Mamba block for 3D inputs (B, D, H, W, C)."""
    def __init__(self, embed_dim, hidden_dim=None):
        super().__init__()
        self.embed_dim = embed_dim
        self.hidden_dim = hidden_dim or embed_dim

        self.input_proj = nn.Linear(embed_dim, self.hidden_dim * 2)
        self.output_proj = nn.Linear(self.hidden_dim, embed_dim)

        self.time_decay = nn.Parameter(torch.zeros(self.hidden_dim))  # learnable decay per channel
    ...
    def forward(self, x):
        ...
        x_proj = self.input_proj(x_flat)  # [B, L, hidden_dim * 2]
        gate, value = x_proj.chunk(2, dim=-1)  # [B, L, hidden_dim]

        gate = self.sigmoid(gate)  # [B, L, hidden_dim]
        u = gate * value           # [B, L, hidden_dim]

        # --- vectorized scan ---
        # precompute decay factors
        decay = (1 - self.time_decay).unsqueeze(0).unsqueeze(0)  # [1,1,C]
        decay_cumprod = torch.cumprod(decay.repeat(B, L, 1), dim=1)  # [B, L, hidden_dim]

        # weighted cumulative sum
        u_weighted = u * decay_cumprod
        out = torch.cumsum(u_weighted, dim=1)  # [B, L, hidden_dim]
        out = out / (decay_cumprod + 1e-6)      # normalize by decay

        # --- end scan ---

        out = self.activation(out)
        out = self.output_proj(out)  # [B, L, embed_dim]

        out = out.view(B, D, H, W, -1)  # reshape back
        return out


class DynamicMambaBlock3D(nn.Module):
    """Dynamic Mamba 3D Block with input-driven dynamic kernel scanning."""
    def __init__(self, embed_dim, hidden_dim=None, conv_kernel_size=3, dynamic_ratio=2):
        super().__init__()
        self.embed_dim = embed_dim
        self.hidden_dim = hidden_dim or embed_dim
        self.dynamic_dim = int(self.hidden_dim * dynamic_ratio)

        self.input_proj = nn.Linear(embed_dim, self.dynamic_dim * 2)
        self.output_proj = nn.Linear(self.dynamic_dim, embed_dim)

        self.dynamic_kernel_gen = nn.Linear(self.dynamic_dim, self.dynamic_dim * conv_kernel_size**3)
        self.conv_kernel_size = conv_kernel_size
        self.sigmoid = nn.Sigmoid()

        self.padding = conv_kernel_size // 2
    ...
    def forward(self, x):
        ...
        # Linear projection
        x_proj = self.input_proj(x_flat)  # [B, L, dynamic_dim * 2]
        gate, value = x_proj.chunk(2, dim=-1)  # [B, L, dynamic_dim]
        gate = self.sigmoid(gate)

        # 生成dynamic卷积核
        dynamic_kernel = self.dynamic_kernel_gen(value)  # [B, L, dynamic_dim * k^3]
        dynamic_kernel = dynamic_kernel.view(B, L, self.dynamic_dim, self.conv_kernel_size**3)  # [B, L, dynamic_dim, k^3]

        # 准备 value特征
        value = value.view(B, D, H, W, self.dynamic_dim).permute(0, 4, 1, 2, 3)  # [B, dynamic_dim, D, H, W]

        # Pad
        value_padded = F.pad(value, (self.padding, self.padding, self.padding, self.padding, self.padding, self.padding))  # 3D padding

        # 3D unfold: 取出每个位置对应的局部k^3 patch
        value_patches = value_padded.unfold(2, self.conv_kernel_size, 1)\
                                     .unfold(3, self.conv_kernel_size, 1)\
                                     .unfold(4, self.conv_kernel_size, 1)  # [B, dynamic_dim, D, H, W, k, k, k]

        value_patches = value_patches.contiguous().view(B, self.dynamic_dim, D, H, W, -1)  # [B, dynamic_dim, D, H, W, k^3]
        value_patches = value_patches.permute(0, 2, 3, 4, 1, 5).contiguous().view(B, L, self.dynamic_dim, self.conv_kernel_size**3)  # [B, L, dynamic_dim, k^3]

        # 每个位置做 dynamic kernel 和局部 patch 的乘积求和
        out = (dynamic_kernel * value_patches).sum(dim=-1)  # [B, L, dynamic_dim]
...
class SwinTransformerBlock3D_Mamba(nn.Module):
    """Swin Transformer Block + Mamba 3D, with flexible order."""
    ...
    def __init__(
        ...
        jump_attention=False,
        frag_bias=False,
        use_mamba_first=True,  # ✨ 新增控制顺序的参数
    ):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.jump_attention = jump_attention
        self.frag_bias = frag_bias
        self.use_mamba_first = use_mamba_first  # ✨ 保存顺序开关

        assert all(0 <= ss < ws for ss, ws in zip(shift_size, window_size)), "shift_size must be in 0 ~ window_size"
        ...
        self.mamba = MinimalMambaBlock3D(dim)  #  MinimalMambaBlock3D DynamicMambaBlock3D
    ...
    def forward(self, x, mask_matrix, resized_window_size=None):
        ...
        if not self.jump_attention:
            if self.use_checkpoint:
                attn_out = checkpoint.checkpoint(self.forward_part1, x, mask_matrix, resized_window_size)
            else:
                attn_out = self.forward_part1(x, mask_matrix, resized_window_size)
        else:
            attn_out = x  # if jump attention

        # Mamba branch (parallel)
        mamba_out = self.mamba(self.norm_mamba(x))
        x = shortcut + self.drop_path(attn_out) + 0.3 * self.drop_path(mamba_out)  # 🔥并行融合
